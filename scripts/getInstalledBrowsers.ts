...
function getCommand(name: string, type: string, path: string) {
  ...
  switch (binaryType) {
    case "flatpak":
      return `flatpak run ${path} --no-input --browser-console`

    case "snap":
      return `snap run ${path} --no-input --browser-console`

    case "native":
      if (type === "chrome") {
        return `web-ext run --target chromium --chromium-binary ${path} --source-dir dist/chrome --no-input --browser-console`
      }
      if (type === "firefox") {
        return `web-ext run --firefox=${path} --source-dir dist/firefox --no-input --browser-console`
      }
      break
